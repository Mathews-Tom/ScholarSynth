...
# Define a type alias for Document to avoid import issues
Document = Any
...
from retrieval.text_splitter import get_parent_child_splitters, get_text_splitter
from retrieval.vector_store import add_documents_to_store, get_vector_store
...
def ingest_with_standard_approach(
    config: SimpleNamespace, documents: List[Document], clear_existing: bool = False
) -> None:
    """
...
def main() -> None:
    ...
    pdf_dir_str = getattr(config, "PDF_SOURCE_DIR", "data/local_pdfs")
    pdf_dir = Path(project_root) / pdf_dir_str
